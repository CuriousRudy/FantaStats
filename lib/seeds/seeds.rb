...
players = File.read("./lib/seeds/players.csv")
...
teams = File.read("./lib/seeds/team_table.csv")
teams = CSV.parse(teams)
team_col = teams.shift

teams.each do |row|
  Team.create("#{team_col[0].to_sym} => #{row[0]}, #{team_col[1].to_sym} => #{row[1]}, #{team_col[2].to_sym} => #{row[2]}, #{team_col[3].to_sym} => #{row[3]}, #{team_col[4].to_sym} => #{row[4]}")
end
